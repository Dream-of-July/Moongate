...
final class MacOSQueueBoundaryTests: XCTestCase {
    ...
    func testQueueManagerWiresPostDownloadQualityGateFallback() throws {
        ...
        // 是否生成 Whisper 的决策统一走 SubtitleSourceDecisionEngine（门每候选只跑一次、按 policy 出计划），
        // 不再用"门(布尔) || 评分器(枚举)"双裁决 OR，也不再在 QueueManager 内单独跑门。
        XCTAssertTrue(source.contains("SubtitleSourceDecisionEngine.assess"))
        XCTAssertTrue(source.contains("SubtitleSourceDecisionEngine.generationPlan"))
        XCTAssertTrue(source.contains("case .generateLocalASRThenChoose:"))
        XCTAssertTrue(source.contains("case .keepPlatformRecordReasons(let reasons):"))
        ...
        XCTAssertFalse(source.contains("platformScore.verdict <= .lowConfidence"))
        XCTAssertFalse(source.contains("shouldGenerateLocalASR"))
        XCTAssertFalse(source.contains("assessPlatformSubtitle"))
    ...
    func testQueueItemDisplaysResolvedSubtitleSourceDetails() throws {
        ...
        XCTAssertFalse(disclosureBody.contains("candidateReports"))
        XCTAssertFalse(disclosureBody.contains("subtitleSourceQuality"))
        XCTAssertFalse(disclosureBody.contains("subtitleCandidateStatus"))
