...
final class MacOSQueueBoundaryTests: XCTestCase {
    ...
    func testQueueManagerWiresPostDownloadQualityGateFallback() throws {
        ...
        XCTAssertTrue(source.contains("case .forcePlatform, .preferPlatform, .cloudASR, .importedFile:"))
        XCTAssertTrue(source.contains("shouldGenerateLocalASR = false"))
        ...
        XCTAssertTrue(source.contains("let platformScore = SubtitleQualityScorer.score"))
        XCTAssertTrue(source.contains("shouldGenerateLocalASR = !verdict.usable || platformScore.verdict <= .lowConfidence"))
        XCTAssertTrue(source.contains("if !shouldGenerateLocalASR"))
