...
0.8.1 之后的字幕质量与发布规范修订。

### 字幕识别修复

- **拉丁字母字幕不再粘词**：修复 Whisper 子词重接（如 `op`+`in`+`ion` → `opinion`）匹配过宽、把相邻的完整词也粘在一起的问题——"at its core, a simple idea" 曾被拼成 "corea"、"we stop in Paris" 曾成 "stopin"。现在只重接真正的子词碎片，英/法/德/意/葡语正常语音保持完整。
- **识别质量判定双端一致**：含不可见格式符的阿拉伯语（从右到左）转写此前可能在 macOS 判为空、在 Windows 不判为空。两端现在对不可见字符处理一致。

### 内部改进

- **统一发布版本面**：所有打包与文档出处（构建脚本、安装器、appcast、README、docs）现在都对齐到单一版本号/构建号真值来源，堵住 0.8.1 遗留的版本分裂缺口。
- **字幕质量评测诚实性**：内部评测框架不再把过期评审证据当作最新，会钳制越界的裁判分数，并在中断时清理临时凭证副本。

## 0.8.1

字幕质量版本：语言优先的字幕选择、统一的字幕源决策、更可靠的本地识别，以及多处真实的"首句字幕出现时机/解析"修复。macOS 与 Windows 核心逻辑保持一致，并新增一套可量化的字幕质量 scorecard。

### 字幕源决策

- **统一决策引擎**：「用哪个源——平台字幕 / 本地 Whisper / 云端」的决策现在是 Ready 页预测与下载后执行共用的单一权威，UI 显示与实际结果不再打架。质量门对每个候选只跑一次（此前会跑两到三次），并把旧的「布尔门 OR 五级评分」双裁决收敛为单一具名质量地板。跨端评分/排序常量统一进共享 fixture，两端各自断言。

### 本地识别（Whisper）

- **语音边界检测不再毁掉音乐**：可选的 VAD 组件实测会在歌唱/音乐内容上丢掉大部分音频（把演唱人声判为非语音）。现已对音乐/歌词档关闭，仅在演讲档保留（演讲档有效）。
- **识别质量低不再让整个任务失败**：出现重复循环或语言误判的转写此前会中止下载；现在仍照常产出字幕，并附「识别可能不可靠」的诚实提示，让你保留结果、自行判断。
- **可选云端识别救场（需手动开启）**：本地识别置信度低、且你已明确配置并同意云端识别时，月之门可对该片段改用云端。未开启则绝不上传音频。

### 字幕时机与解析修复

- **首句字幕准时出现**：修复 YouTube 自动字幕的解析 bug——时间行与文本行之间的空格占位行会导致开头那条字幕被整条丢弃、下一条被错位推迟。现在首句字幕在语音真正开始时出现。
- **首条 lead-in**：视频的第一条字幕不再被全局起点微调往后推。
- **演讲断句更准**：本该断句的真实停顿不再被重新合并掉，演讲字幕的断行更贴近人工字幕（已对照人工字幕验证）。音乐/动漫断行保持不变。
- **Whisper 口吃清理**：识别产生的即时重复词组（如「I've got to leave I've got to leave I've got to leave」）在翻译前折叠，所有语言生效。

### 质量度量

- **可量化 scorecard**：四维字幕质量标准（识别 / 分段 / 翻译 / 源决策，各 0–100、≥80 为优秀门禁），建在真实缓存样本上，并区分"模型置信度启发式"与"金标准（人工参考 / 声学 / 裁判）验证"。
...
- 新增决策引擎（两端 fixture 契约）、VTT 空格占位解析修复、首条时机、仅演讲档的合并间隙修复、云端救场流程、口吃折叠等跨端测试，以及语言推荐器与平台字幕质量门（含「质量门绝不拿时序判 whisper」防回归）。既有铁律与时序防回归全部保持绿。
