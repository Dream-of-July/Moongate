...
public final class TaskControlToken: @unchecked Sendable {
    ...
    private static let signalQueue = DispatchQueue(label: "moongate.taskcontrol.signal", qos: .userInitiated)
    ...
    public func gate() async throws {
        if Task.isCancelled || isCancelled { throw MoongateError.cancelled }
        while true {
            let waitNeeded: Bool = {
