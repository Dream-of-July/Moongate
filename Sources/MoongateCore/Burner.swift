...
public struct FFmpegBurner: SubtitleBurner {
    ...
    static func encoderAvailable(_ encoder: String, ffmpeg: String) -> Bool {
        ...
        for token in ["libx265", "libx264", "libsvtav1", "hevc_videotoolbox"] where text.contains(token) {
            found.insert(token)
        }
        encoderCacheLock.lock()
    ...
    public func burn(
        ...
        let sdrVideo = ["-c:v", "libx264", "-crf", "20", "-preset", "medium",
                        "-pix_fmt", "yuv420p",
                        "-maxrate", "\(maxrateK)k", "-bufsize", "\(maxrateK * 2)k"]

        // HDR 源：用 libx265 10-bit 保 HDR；x265 不可用则回退 tonemap→SDR(libx264)。
        let x265Available = Self.encoderAvailable("libx265", ffmpeg: ffmpeg)
        let useHDR = probe.isHDR && x265Available
        let softwareVideo: [String]
        let videoFilter: String
        if useHDR {
            softwareVideo = Self.hdrVideoArgs(
            ...
            // x265 不可用：先 tonemap 成 SDR 再叠字幕（画质降级，但仍能烧录）。
    ...
    struct ASSLayout: Equatable {
        ...
        init(aspect: Double) {
            ...
            let readableWidth = Double(playResX - marginH * 2)
            let latinCapacity = Int((readableWidth / (Double(originalSize) * 0.55)).rounded(.down))
    ...
    static func makeASS(cues: [SubtitleCue], aspect: Double = 16.0 / 9.0) -> String {
        ...
        for cue in cues {
            ...
            let rawOtherLines = lines.filter { !Self.containsCJK($0) }
            let otherLines: [String]
            if let capacity = layout.latinWrapCapacity, !rawOtherLines.isEmpty {
                let joined = rawOtherLines.joined(separator: " ")
                otherLines = wrapLatinLine(joined, capacity: capacity)
            } else {
                otherLines = rawOtherLines
            }
            ...
            } else if !cjkLines.isEmpty {
                text = cjkLines.joined(separator: "\\N")
            } else {
                // 纯原文（无中文）条目：也按词折行，避免长英文行在窄列被 libass 乱断。
                text = otherLines.joined(separator: "\\N")
            }
