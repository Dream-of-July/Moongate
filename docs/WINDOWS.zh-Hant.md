...
- **MoongateCore**（C#，.NET 10）— 以 Swift 版 `MoongateCore` + `QueueManager` 逐行為基準移植的核心庫：yt-dlp 封裝、字幕解析/清洗/翻譯、ffmpeg 燒錄、佇列與並行槽位、暫停/取消、設定與 cookies。附 470 個單元測試，在 macOS/Windows 上均可全量執行。
