...
```bash
./build-windows.sh            # 输出 ~/Downloads/Moongate-Windows-Setup-v0.7.4.exe 和 .sha256
```
...
1. 双击 `Moongate-Windows-Setup-v0.7.4.exe` → 安装到默认用户目录（无 UAC 弹窗）。
