...
發布 / 更新包和本機安裝建置分開：`./make-sparkle-zip.sh` 產生 Sparkle 使用的 `Moongate-macOS-v0.8.2.zip`，上傳到對應 GitHub Release 後，再用 `./make-appcast.sh` 寫入簽名後的 `docs/appcast.xml`。`./make-dmg.sh` 仍保留為手動拖曳安裝包。
...
產出 `Moongate-Windows-Setup-v0.8.2.exe`（附 `.sha256`）。雙擊安裝、無需系統管理員權限，首次啟動自動下載 yt-dlp / ffmpeg / deno。詳見 [docs/WINDOWS.zh-Hant.md](docs/WINDOWS.zh-Hant.md)。
