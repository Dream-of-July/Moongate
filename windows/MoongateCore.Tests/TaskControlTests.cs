...
public class TaskControlTests
{
    ...
    public async Task Gate_CancelWhilePaused_ThrowsCancelled()
    {
        ...
        await Task.Delay(50);
        token.Cancel();
        var ex = await Assert.ThrowsAsync<MoongateException>(() => gate.WaitAsync(TimeSpan.FromSeconds(5)));
        Assert.Equal(MoongateErrorKind.Cancelled, ex.Kind);
    ...
    public async Task Gate_AlreadyCancelled_ThrowsImmediately()
    {
        var token = new TaskControlToken();
        token.Cancel();
        var ex = await Assert.ThrowsAsync<MoongateException>(() => token.GateAsync());
        Assert.Equal(MoongateErrorKind.Cancelled, ex.Kind);
    }
